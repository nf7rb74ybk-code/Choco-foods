/* CHOCO SHIP - GPS anti-hang guard v1 */
'use strict';
(function(){
  const boot=()=>{
    const btn=document.getElementById('shipperGpsButton');
    if(!btn||btn.dataset.gpsGuard==='1') return;
    ...
    btn.addEventListener('click',function(e){
      ...
      navigator.geolocation.getCurrentPosition(p=>{
        ...
        if(!Number.isFinite(lat)||!Number.isFinite(lng)){set('❌ GPS trả về vị trí không hợp lệ.');return}
        window.__CHOCO_GPS_GUARD__={lat,lng,at:Date.now()};window.currentGPS={lat,lng};
        set('🟢 GPS đã bật • '+lat.toFixed(6)+', '+lng.toFixed(6),true);
        try{localStorage.setItem('choco_shipper_gps',JSON.stringify({lat,lng,at:Date.now()}))}catch{}
      },e=>{
        if(done)return;done=true;clearTimeout(timer);
        if(e?.code===1)set('❌ Quyền vị trí bị chặn. Cho phép Safari dùng vị trí.');
        else if(e?.code===2)set('❌ Không xác định được vị trí. Bật Dịch vụ định vị.');
        else set('❌ GPS hết thời gian chờ. Thử lại.');
