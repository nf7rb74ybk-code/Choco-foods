...
(function(){
  ...
  function render(){
    ...
    for(var ty=minY;ty<=maxY;ty++)for(var tx=minX;tx<=maxX;tx++){
      ...
      var img=document.createElement('img');img.alt='';img.draggable=false;img.decoding='async';img.loading='eager';
    ...
    if(pin){var px=xFromLng(pin.lng,z)*tile-left,py=yFromLat(pin.lat,z)*tile-top;pinEl.style.left=px+'px';pinEl.style.top=py+'px';pinEl.style.display='block'}
  }
  function selectAt(clientX,clientY){
    var r=el.getBoundingClientRect(),w=el.clientWidth||r.width,h=el.clientHeight||r.height,cx=xFromLng(center.lng,z),cy=yFromLat(center.lat,z);
