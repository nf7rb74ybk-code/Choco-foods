/* CHOCO SHIP — Customer Checkout v4
   Reads persisted cart and refreshes expired customer sessions before RPC checkout.
*/
'use strict';
(function(){
  ...
  async function checkout(e){
    ...
    try{
      ...
      const body={p_restaurant_id:restaurantId,p_items:items,p_name:name,p_phone:phone,p_address:address,p_note:note,p_payment:payment,p_latitude:g.lat,p_longitude:g.lng};
      ...
      if((r.status===401||r.status===403)&&localStorage.getItem('choco_refresh_token')){
        t=await token();r=await fetch(U+'/rest/v1/rpc/create_customer_order',{method:'POST',headers:{apikey:K,Authorization:'Bearer '+t,'Content-Type':'application/json',Accept:'application/json'},body:JSON.stringify(body)});data=await r.json().catch(()=>null);
      }
    ...
    }catch(err){console.error('[CHOCO CHECKOUT v4]',err);alert('❌ KHÔNG GỬI ĐƯỢC ĐƠN.\n\n'+err.message)}
