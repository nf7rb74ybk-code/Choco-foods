...
self.addEventListener('message',event=>{
  if(event.data?.type!=='CHOCO_TEST_NOTIFICATION')return;
  event.waitUntil(self.registration.showNotification('🔔 CHOCO SHIP - TEST',{body:'Push Service Worker của khách đang hoạt động.',icon:'./icon-512x512.png?v=20260904',badge:'./icon-512x512.png?v=20260904',tag:'customer-push-test',renotify:true,silent:false,data:{url:'./customer.html',code:'TEST',status:'TEST'},timestamp:Date.now()}));
});
