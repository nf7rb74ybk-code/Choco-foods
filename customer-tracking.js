...
(function(){
  ...
  function inject(){
    ...
    const style=document.createElement('style');style.textContent='#customerTrackingBox{background:#fff;border-radius:15px;padding:14px;margin:12px 0;box-shadow:0 2px 8px #ddd}#customerTrackingMap{height:300px;border-radius:12px;margin-top:10px}.ct-status{padding:10px;border-radius:10px;background:#eff6ff;color:#1d4ed8;line-height:1.5}.ct-btn{width:100%;border:0;border-radius:10px;padding:12px;margin-top:10px;background:#1677ff;color:#fff;font-weight:800}.ct-muted{font-size:12px;color:#666;margin-top:7px}';document.head.appendChild(style);
    const box=document.createElement('section');box.id='customerTrackingBox';box.innerHTML='<div style="font-size:19px;font-weight:800">🚚 Theo dõi Shipper</div><div id="customerTrackingStatus" class="ct-status" style="margin-top:10px">📦 Chưa có đơn đang theo dõi.</div><div id="customerTrackingMap"></div><button id="customerTrackingRefresh" class="ct-btn">🔄 CẬP NHẬT VỊ TRÍ</button><div class="ct-muted">🚚 Marker Shipper di chuyển theo GPS. Đường chấm là lịch sử vị trí đã ghi nhận.</div>';
    const container=document.querySelector('.container')||document.body;container.insertBefore(box,container.firstChild);document.getElementById('customerTrackingRefresh').onclick=()=>refresh(true);
  }
  ...
  async function loadTrail(shipperId){
    try{
      const r=await fetch(SB+'/rest/v1/shipper_gps_history?shipper_id=eq.'+encodeURIComponent(shipperId)+'&select=latitude,longitude,recorded_at&order=recorded_at.asc&limit=300',{headers:{apikey:KEY,Accept:'application/json'}});
      if(!r.ok)return;
      const rows=await r.json();trailPoints=rows.map(x=>[Number(x.latitude),Number(x.longitude)]).filter(x=>Number.isFinite(x[0])&&Number.isFinite(x[1]));
      if(trailPoints.length>1&&map){if(trail)trail.setLatLngs(trailPoints);else trail=L.polyline(trailPoints,{dashArray:'6 8'}).addTo(map)}
    }catch(e){}
  }
