/* CHOCO SHIP - CUSTOMER LIVE TRACKING v4 */
'use strict';
(function(){
  ...
  function auth(){
    const o=order();
    const expected=String(o?.customer_id||localStorage.getItem('choco_user_id')||'');
    ...
    let chosen=candidates.find(x=>expected&&String(jwt(x.token)?.sub||x.user?.id||'')===expected)||candidates[0]||null;
    if(!chosen)return{token:'',id:expected,refresh:'',email:localStorage.getItem('choco_email')||'',source:'none'};
    const p=jwt(chosen.token);
    return{token:chosen.token,id:String(p?.sub||chosen.user?.id||expected||''),refresh:chosen.refresh||'',email:chosen.user?.email||localStorage.getItem('choco_email')||'',source:chosen.source};
  }
  ...
  async function tracking(){
    ...
    if(r.status===401){
      if(await refreshAccess()){const b=auth();r=await fetch(SB+'/rest/v1/rpc/get_my_shipper_tracking',{method:'POST',headers:{apikey:KEY,Authorization:'Bearer '+b.token,'Content-Type':'application/json',Accept:'application/json'},body:JSON.stringify({p_order_code:o?.code||''})});}
      else return{auth:false,expired:true};
    }
    if(!r.ok)throw Error('Tracking HTTP '+r.status+': '+await r.text());
    const rows=await r.json();
    return{auth:true,data:Array.isArray(rows)?rows[0]:rows,uid:a.id,expected:String(o?.customer_id||'')};
  }
