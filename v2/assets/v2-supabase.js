// CHOCO SHIP V2 — Supabase Auth + production data layer
...
const V2_API={orders:'/rest/v1/orders',logs:'/rest/v1/choco_v2_test_logs',profiles:'/rest/v1/choco_v2_auth_profiles'};
...
function v2GetUserId(){return v2GetSession()?.user?.id||null}
function v2Headers(extra={}){const token=v2GetAccessToken();return{apikey:V2_SUPABASE_CONFIG.anonKey,Authorization:token?`Bearer ${token}`:`Bearer ${V2_SUPABASE_CONFIG.anonKey}`,'Content-Type':'application/json',...extra}}
async function v2Request(path,options={}){if(!v2Ready())throw new Error('CHOCO SHIP V2 chưa được cấu hình Supabase.');const res=await fetch(V2_SUPABASE_CONFIG.url.replace(/\/$/,'')+path,{...options,headers:v2Headers(options.headers||{})});const text=await res.text();if(!res.ok)throw new Error(text||`HTTP ${res.status}`);return text?JSON.parse(text):null}
