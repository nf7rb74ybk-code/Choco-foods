// CHOCO SHIP V2 — Supabase Auth session + TEST data layer
...
function v2GetAccessToken(){return v2GetSession()?.access_token||null}
function v2Headers(extra={}){const token=v2GetAccessToken();return{apikey:V2_SUPABASE_CONFIG.anonKey,Authorization:`Bearer ${token||V2_SUPABASE_CONFIG.anonKey}`,'Content-Type':'application/json',...extra}}
async function v2Request(path,options={}){if(!v2Ready())throw new Error('V2 TEST chưa được cấu hình Supabase.');const res=await fetch(V2_SUPABASE_CONFIG.url.replace(/\/$/,'')+path,{...options,headers:v2Headers(options.headers||{})});const text=await res.text();if(!res.ok)throw new Error(text||`HTTP ${res.status}`);return text?JSON.parse(text):null}
...
async function v2Log(orderId,event,payload={}){return v2Request(V2_API.logs,{method:'POST',headers:{Prefer:'return=minimal'},body:JSON.stringify({order_id:orderId,event,payload})})}
...
async function v2RefreshSession(refreshToken){return v2Request('/auth/v1/token?grant_type=refresh_token',{method:'POST',body:JSON.stringify({refresh_token:refreshToken})})}
async function v2GetProfile(accessToken,userId){const controller=new AbortController();const timer=setTimeout(()=>controller.abort(),7000);try{const res=await fetch(V2_SUPABASE_CONFIG.url+`${V2_API.profiles}?select=user_id,display_name,role,phone,active&user_id=eq.${encodeURIComponent(userId)}&active=eq.true`,{signal:controller.signal,headers:{apikey:V2_SUPABASE_CONFIG.anonKey,Authorization:`Bearer ${accessToken}`,'Content-Type':'application/json'}});const text=await res.text();if(!res.ok)throw new Error(text||`HTTP ${res.status}`);return text?JSON.parse(text):null}catch(e){if(e.name==='AbortError')throw new Error('Supabase xác thực quá thời gian.');throw e}finally{clearTimeout(timer)}}
