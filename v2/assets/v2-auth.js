...
async function v2RequireRole(role){try{const p=await Promise.race([v2RestoreAuth(),new Promise((_,reject)=>setTimeout(()=>reject(new Error('Xác thực quá thời gian. Vui lòng đăng nhập lại.')),10000))]);if(!p||p.role!==role){v2ClearIdentity();location.replace('./login.html?role='+encodeURIComponent(role));return null}return p}catch(e){v2ClearIdentity();location.replace('./login.html?role='+encodeURIComponent(role)+'&error=auth_timeout');return null}}
