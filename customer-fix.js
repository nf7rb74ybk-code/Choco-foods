...
(function () {
  ...
  function clearAuth() {
    ["choco_access_token", "choco_user_id", "choco_role", "choco_email"].forEach(function (k) {
      localStorage.removeItem(k);
    });
  }

  function goLogin() {
    if (window.__chocoCustomerRedirecting) return;
    window.__chocoCustomerRedirecting = true;
    clearAuth();
    var next = "customer.html";
    window.location.replace("index.html?next=" + encodeURIComponent(next) + "&t=" + Date.now());
  }
  ...
  async function resolveCustomer() {
    ...
    /* customer.html is not a login page. If there is no real Supabase session,
       send the user back to the single CHOCO SHIP login page. */
    if (!x.token || !x.id) {
  ...
  window.getCustomerId = function () {
    ...
    var role = localStorage.getItem("choco_role") || "";
    return x.id && stored === x.id && role === "customer" ? x.id : "";
  };
  window.chocoResolveCustomer = resolveCustomer;

  /* Validate the session immediately. customer.html remains a normal order
     page, while index.html remains the only authentication page. */
  (async function () {
    var result = await resolveCustomer();
    if (!result.ok) {
      console.warn("[CHOCO SHIP] customer identity:", result.error);
      if (result.error === "NO_SESSION" || result.error === "NOT_CUSTOMER") goLogin();
    }
  })();
  ...
  if (!window.__chocoCustomerOrderIdentityInstalled) {
    ...
    window.fetch = async function (input, init) {
      ...
      if (method === "POST" && /\/rest\/v1\/orders(?:\?|$)/i.test(url) && init.body) {
        ...
        if (!result.ok) {
          if (result.error === "NO_SESSION" || result.error === "NOT_CUSTOMER") goLogin();
          throw new Error(result.error || "Không xác định được tài khoản khách.");
        }
  ...
  async function reverseViaSupabase(lat, lng) {
    ...
    var r = await originalFetch(REVERSE_URL, {
      method: "POST",
      headers: headers,
  ...
  async function reverseGeocode(lat, lng) {
    try {
      return await reverseViaSupabase(lat, lng);
    } catch (e) {
  ...
  function finish(lat, lng) {
    ...
    reverseGeocode(lat, lng).then(function (a) {
      if (a) {
        var el = document.getElementById("address");
        if (el && (!el.value || el.value.indexOf("GPS:") === 0)) setAddress(a);
        setText("cartGpsText", "📍 " + a + "\nGPS: " + gpsText(lat, lng));
      } else {
        setText("cartGpsText", "📍 GPS: " + gpsText(lat, lng) + "\n⚠️ Chưa tìm được tên địa chỉ tự động.");
      }
    }).catch(function () {});
  }
  ...
  function extractGPS() {
    ...
    for (var i = 0; i < els.length; i++) {
      var text = els[i] ? els[i].innerText || "" : "";
