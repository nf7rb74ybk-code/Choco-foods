...
(function(){
  ...
  async function enable(){const btn=$('adminPush');if(btn)btn.disabled=true;try{
    ...
    if($('adminPushTest'))$('adminPushTest').disabled=false;
  ...
  async function testPush(){const btn=$('adminPushTest');if(btn)btn.disabled=true;dbg('⏳ Đang gửi TEST tới Admin...');try{
    const r=await fetch(SB+'/functions/v1/send-push?mode=test_admin&user_id='+encodeURIComponent(UID),{headers:{apikey:KEY,Accept:'application/json'}});
    const text=await r.text();let j={};try{j=JSON.parse(text)}catch{};
    if(!r.ok||!j.ok)throw Error(j.error||text||('HTTP '+r.status));
    dbg('✅ Đã gửi TEST. Target: '+(j.target_count||0)+'<br>Kiểm tra thông báo trên iPhone.');
    $('adminPushStatus').innerHTML='✅ <b>TEST PUSH ĐÃ GỬI</b>';
