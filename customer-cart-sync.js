/* CHOCO SHIP — Customer cart live sync v2 */
'use strict';
(function(){
  ...
  async function sync(){
    ...
    try{
      ...
      // Never treat an empty response as proof that every cart item was deleted.
      // RLS/session timing can legitimately return zero visible rows.
      ...
      for(const x of c){
        ...
        if(m.is_available!==true){removed++;changed=true;continue}
        ...
        if(item.restaurantId!==Number(x.restaurantId)||item.name!==String(x.name||'')||item.price!==oldPrice)changed=true;
        next.push(item);
      }
