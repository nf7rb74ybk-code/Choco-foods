...
let liveData=[];
const originalAdd=typeof window.add==='function'?window.add:null;
...
function addLive(rid,fid){
 const r=liveData.find(x=>String(x.id)===String(rid)),f=r?.foods.find(x=>String(x.id)===String(fid));
 if(!r||!f)return;
 if(typeof originalAdd==='function'){originalAdd(rid,fid);return;}
 if(!Array.isArray(window.cart))window.cart=[];
 const old=window.cart.find(x=>String(x.foodId)===String(f.id));
 if(old)old.qty=Math.min(99,Number(old.qty||1)+1);else window.cart.push({restaurantId:r.id,restaurant:r.name,foodId:f.id,name:f.name,price:f.price,qty:1});
 if(typeof window.updateCart==='function')window.updateCart();
 try{localStorage.setItem('choco_customer_cart_v1',JSON.stringify(window.cart))}catch{}
 if(typeof window.renderCart==='function')window.renderCart();
 alert('✅ Đã thêm '+f.name);
}
