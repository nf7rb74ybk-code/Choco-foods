...
import {
  DECISION_LEDGER_MODE,
  appendDecision,
  findDecision,
  decisionLedgerSafetyCheck,
} from '../audit/lab-approval-decision-ledger.js';

const entry = createAuditHistoryEntry({
  queueId: 'LAB-QUEUE-46',
  proposal: { action: 'ASSIGN_SHIPPER', target: 'LAB-ORDER-046' },
...
const checks = {
  ...
  ledger_entry_created: ledger.length === 1,
  record_id_traceable: found?.record_id === record.record_id,
  manifest_checksum_traceable: found?.manifest_checksum === manifest.checksum,
  requester_traceable: found?.requested_by === 'LAB_ADMIN_TEST',
  decision_traceable: found?.decision === 'APPROVAL_RECORDED_BUT_EXPORT_DISABLED',
  timestamp_traceable: typeof found?.recorded_at === 'string' && found.recorded_at.length > 0,
  ...
  lookup_returns_same_record: found?.record_id === record.record_id,
  safety_check_passes: decisionLedgerSafetyCheck(ledger) === true,
};

const failed = Object.entries(checks).filter(([, ok]) => !ok);
const report = {
  suite: 'CHOCO_AUTO_STEP_46_LAB_APPROVAL_DECISION_LEDGER',
