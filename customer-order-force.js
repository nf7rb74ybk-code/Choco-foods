/* CHOCO SHIP — CUSTOMER ORDER FORCE v20 */
'use strict';
(function(){
  if(window.__CHOCO_ORDER_FORCE_V20__)return;
  window.__CHOCO_ORDER_FORCE_V20__=true;
  ...
  function bind(){const b=document.getElementById('orderButton');if(!b)return;b.type='button';b.disabled=false;b.style.pointerEvents='auto';b.removeAttribute('onclick');if(!b.__chocoOrderBoundV20__){b.__chocoOrderBoundV20__=true;b.addEventListener('click',function(e){e.preventDefault();e.stopPropagation();if(!loadCheckout()){setTimeout(()=>{if(typeof window.createOrder==='function')window.createOrder(e);else alert('❌ Không tải được chức năng đặt đơn. Vui lòng kiểm tra kết nối mạng rồi thử lại.')},1000);return}try{window.createOrder(e)}catch(err){console.error('[CHOCO ORDER FORCE]',err);alert('❌ Lỗi đặt đơn: '+(err?.message||err))}},false)}loadCheckout()}
