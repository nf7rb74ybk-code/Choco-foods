/* CHOCO SHIP — CUSTOMER ORDER FORCE v18
 * Ensures checkout/tracking/chat assets are loaded on the production customer page.
 */
'use strict';
(function(){
  if(window.__CHOCO_ORDER_FORCE_V18__)return;
  window.__CHOCO_ORDER_FORCE_V18__=true;
  ...
  let loadingCheckout=false,loadingTracking=false,loadingChat=false;
  ...
  function bind(){const b=document.getElementById('orderButton');if(!b)return;b.type='button';b.disabled=false;b.style.pointerEvents='auto';b.removeAttribute('onclick');if(!b.__chocoOrderBoundV18__){b.__chocoOrderBoundV18__=true;b.addEventListener('click',function(e){e.preventDefault();e.stopPropagation();if(!loadCheckout()){setTimeout(()=>{if(typeof window.createOrder==='function')window.createOrder(e);else alert('❌ Không tải được chức năng đặt đơn. Vui lòng kiểm tra kết nối mạng rồi thử lại.')},1000);return}try{window.createOrder(e)}catch(err){console.error('[CHOCO ORDER FORCE]',err);alert('❌ Lỗi đặt đơn: '+(err?.message||err))}},false)}loadCheckout()}
  ...
  function boot(){loadTracking();bind();addOrderNav();loadChat();[300,1000,2000,3500].forEach(ms=>{setTimeout(loadTracking,ms);setTimeout(bind,ms);setTimeout(addOrderNav,ms);setTimeout(loadChat,ms)})}
